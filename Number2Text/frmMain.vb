...
Friend Class frmMain
    ...
    Private Sub cmdGuess_Click(eventSender As System.Object, eventArgs As System.EventArgs) Handles cmdGuess.Click
...
ReStart:
            ...
            txtNumber.Text = rndNumber

            Dim msg As String = String.Format("Nivel: {0}{1}{1}Introduce la forma númerica de {1}{2}", level, Environment.NewLine, lblResult.Text)
    ...
    Private Sub AutoCount(state As Object)
        ...
        Dim sNumber As String = GetIntPart(txtNumber.Text).Replace(",", "")
        ...
        txtNumber.Invoke(New MethodInvoker(Sub() txtNumber.Text = sNumber))
    ...
    Private Sub txtNumber_TextChanged(eventSender As System.Object, eventArgs As System.EventArgs) Handles txtNumber.TextChanged
        Static IsBusy As Boolean
        If IsBusy Then Exit Sub
        IsBusy = True

        If txtNumber.Text = "." Then txtNumber.Text = "0.0"

        Dim intPart As String = GetIntPart(txtNumber.Text)
        Dim frcPart As String = GetFracPart(txtNumber.Text)
        Dim selStart As Integer = txtNumber.SelectionStart - System.Text.RegularExpressions.Regex.Matches(intPart, ",").Count
        ...
        Else
            ...
            txtNumber.Text = sNumber + "." + frcPart
            txtNumber.SelectionStart = IIf(Of Integer)(selStart <= 0, 1, selStart)

            ' Handle decimals
            lblResult.Text = Num2Txt(intPart.Replace(",", ""))
            If Val(frcPart) <> 0 Then
                lblResult.Text = lblResult.Text + " punto"
                For i As Integer = 0 To frcPart.Length - 1
                    If frcPart.Substring(i, 1) <> "0" Then Exit For
                    lblResult.Text = lblResult.Text + " Cero"
                Next i

                ' Remove zeros at the right
                frcPart = frcPart.TrimEnd("0"c)
                lblResult.Text = String.Format("{0} {1}", lblResult.Text, Num2Txt(frcPart))

                ' Alternate Display
                Dim f As String = Num2Frac(CSng(txtNumber.Text.Replace(",", "")))
                If f <> "" Then
                    Dim p As String() = f.Split("/"c)
                    Dim n As String = Num2Txt(p(0))
                    Dim d As String = Num2Txt(p(1))

                    lblResult.Text = String.Format("{0}{1}ó{1}{2} sobre {3}", lblResult.Text, Environment.NewLine, n, d)
                End If
            ElseIf intPart = "42" Then
                lblResult.Text = String.Format("{0}{1}ó{1}El significado de la vida, el universo y todo lo demás...", lblResult.Text, Environment.NewLine)
            End If

            If frcPart = "0" AndAlso intPart.Replace(",", "").Length <= 4 Then
                If intPart = "" Then intPart = "0"

                lblResult.Text += vbCrLf + ToRomanNumeral(Integer.Parse(intPart.Replace(",", "")))
            End If
        End If
    ...
    Private Sub txtNumber_KeyPress(eventSender As System.Object, eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles txtNumber.KeyPress
